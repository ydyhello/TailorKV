...
from matmul import triton_bmm_fA_qB_outer,cuda_bmm_fA_qB_outer
...
def test_bmm_speed():
	BS, nh, T, D = 1, 32, 16000, 128
	bits = 2
	key_state = torch.randn((BS, 8, T, D), device='cuda', dtype=torch.float16)
	val_state = torch.randn((BS, 8, T, D), device='cuda', dtype=torch.float16)
	group_size = 64
	query_len = 1
	...
	stmt = 'cuda_bmm_fA_qB_outer(group_size, query_state, code, scale, mn, bits)'
	...
	stmt = 'cuda_bmm_fA_qB_outer(group_size, attn_weight, code, scale, mn, bits)'
